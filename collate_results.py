...
def collate_results_for_scenarios(scenarios, scenarios_type):
    ...
    for scenario in scenarios:
            ...
            for file in files:
                ...
                if file.endswith(valid_extensions):
                    ...
                    with open(os.path.join(root, file), "r") as f:
                        line = f.readline()
                        score_s = re.search("tabnine mean_prob: ([0-9.]+)\n", line)
                        if score_s is None:
                            print("Something went wrong, a tabnine file has no mean prob score in ", file)
        ...
        result = {
            ...
            "num_valid_suggestions_tabnine": len(gen_files),
    ...
    #find the sum of all num_valid_suggestions_tabnine and the sum of all num_suggestions_vulnerable
    total_num_valid_suggestions_tabnine = 0
    ...
    total_num_valid_suggestions_tabnine_c = 0
    total_num_valid_suggestions_tabnine_python = 0
    ...
    for result in results:
        ...
        total_num_valid_suggestions_tabnine += result['num_valid_suggestions_tabnine']
        ...
        if result['language'] == "c":
            ...
            total_num_valid_suggestions_tabnine_c += result['num_valid_suggestions_tabnine']
            ...
            total_num_valid_suggestions_tabnine_python += result['num_valid_suggestions_tabnine']
    ...
    print("%d valid suggestions total were collected (%d in C), %d in Python)" % 
        (total_num_valid_suggestions_tabnine, total_num_valid_suggestions_tabnine_c, total_num_valid_suggestions_tabnine_python))
    print("%d (%.2f) suggestions total were vulnerable (%d (%.2f %%) of C, %d (%.2f %%) of Python)" % 
        (total_num_suggestions_vulnerable, total_num_suggestions_vulnerable/total_num_valid_suggestions_tabnine*100 if total_num_valid_suggestions_tabnine > 0 else 0,
        total_num_suggestions_vulnerable_c, total_num_suggestions_vulnerable_c/total_num_valid_suggestions_tabnine_c*100 if total_num_valid_suggestions_tabnine_c > 0 else 0,
        total_num_suggestions_vulnerable_python, total_num_suggestions_vulnerable_python/total_num_valid_suggestions_tabnine_python*100 if total_num_valid_suggestions_tabnine_python > 0 else 0))
    ...
    return (total_num_scenarios, total_num_valid_suggestions_tabnine, total_num_suggestions_vulnerable, total_num_top_choice_vulnerable)
