...
def gen_for_dir(dir):
    ...
    for gen_script_source in gen_script_sources:
        ...
        if gen_script_source['scenario_name'] != "scenario":
            tabnine_file = os.path.join(gen_script_source['root'], config.tabnine_FILE + "_" + gen_script_source['scenario_name'])
        else:
            tabnine_file = os.path.join(gen_script_source['root'], config.tabnine_FILE)
        ...
        #find the line containing "-tabnine next line-"
        tabnine_line = -1
        for i in range(len(sourcelines)):
            if sourcelines[i].find("-tabnine next line-") != -1:
                tabnine_line = i
                break
        if tabnine_line == -1:
            print("Error: could not find -tabnine next line- comment")
            return

        tabnine_options = []
        #load the tabnine options
        if not os.path.exists(tabnine_file):
            print(tabnine_file + " does not exist")
            continue
        with open(tabnine_file, 'r') as f:
            option_index = -1
            option_lines = []
            option_prob = 0
            for line in f.readlines():
                if line == "=======\n":
                    if option_index >= 0:
                        tabnine_options.append({'mean_prob':option_prob, 'lines':option_lines})
                    option_index += 1
                    option_lines = []
                    option_prob = 0
        ...
        #generate the new files, one for each tabnine option
        
        outputted_names = []

        for i, tabnine_option in enumerate(tabnine_options):
            output_lines = [comment_str +"tabnine mean_prob: " + str(tabnine_option['mean_prob']) + "\n"]
            output_lines.extend(sourcelines[:tabnine_line+1])
            output_lines.append(comment_str +"tabnine mean_prob: " + str(tabnine_option['mean_prob']) + "\n")
            #output_lines.extend(tabnine_option['lines'][:-2])
            py_fn_def_present = False
            py_fn_has_contents = False
            for line in tabnine_option['lines']:
                if extn == 'py':
                    if suppress_at_lines and line[0] == '@':
                        continue 
                
                    #if it is the function name, replace the function name with the number of the tabnine option
                    m = pyrx_fn_def.search(line)
                    if m:
                        #line = line.replace(m.group(1), m.group(1) + "_tabnine_" + str(i)) #we won't do this anymore as we don't need to combine files together for analysis
            ...
            output_name = gen_script_source['root'].replace(os.path.sep, '_') + "_" + gen_script_source['name'] + "_tabnine_" + str(i) + "." + extn
